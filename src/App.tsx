...
function App() {
  ...
  const numCards = barrasActivas.length;
  const timerApilado = numCards > 5;
  const cardStyle = { '--num-cards': numCards } as React.CSSProperties;
        ...
        <section className={`active-bars-container${numCards <= 2 ? ' few-cards' : ''}`}>
          {[...barrasActivas].reverse().map((b) => {
            ...
            return (
              <div
                key={b.id}
                className={`bar-card ${esAlerta ? 'alert-pulse' : ''}`}
                style={cardStyle}
              >
                <button
                ...
                <div className={`progress-wrapper${timerApilado ? ' stacked' : ''}`}>
